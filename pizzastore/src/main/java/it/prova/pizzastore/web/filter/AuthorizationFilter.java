...
public class AuthorizationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!AuthenticationFilter.isPathInWhiteList(pathAttuale) && isPathForOnlyPizzaiolo(pathAttuale)
				&& !AuthenticationFilter.getUserLoggedIn(httpRequest).isAPizzaiolo()) {
			httpResponse.sendRedirect(httpRequest.getContextPath() + "/home?operationResult=NOT_ALLOWED");
			return;
		}

		if (!AuthenticationFilter.isPathInWhiteList(pathAttuale) && isPathForOnlyAFattorino(pathAttuale)
				&& !AuthenticationFilter.getUserLoggedIn(httpRequest).isAFattorino()) {
			httpResponse.sendRedirect(httpRequest.getContextPath() + "/home?operationResult=NOT_ALLOWED");
			return;
		}
	...
	private static boolean isPathForOnlyPizzaiolo(String requestPath) {
	...
	private static boolean isPathForOnlyAFattorino(String requestPath) {
